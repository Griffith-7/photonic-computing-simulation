...
std::vector<MZIRecord> ReckDecomposer::decompose(const Eigen::MatrixXcd& U) {
    int N = U.rows();
...
Eigen::MatrixXcd ReckDecomposer::reconstruct(const std::vector<MZIRecord>& mzis, int N) {
    Eigen::MatrixXcd result = Eigen::MatrixXcd::Identity(N, N);

    for (int k = 0; k < static_cast<int>(mzis.size()); k++) {
        const auto& m = mzis[k];
        double ct = std::cos(m.theta);
        double st = std::sin(m.theta);

        for (int i = 0; i < N; i++) {
            Complex v0 = result(i, m.mode1);
            Complex v1 = result(i, m.mode2);
            result(i, m.mode1) = ct * v0 - IM * std::exp(-IM * m.phi) * st * v1;
            result(i, m.mode2) = -IM * std::exp(IM * m.phi) * st * v0 + ct * v1;
        }
    }
...
double ReckDecomposer::error(const Eigen::MatrixXcd& U_target,
